...
<title><?php if(trim($search) != "") echo $search." - جستجوی "; ?>کتاب های <?php echo $main_title; ?> Books</title>
<meta name="description" content="بخش کتاب های موتور جستجوی فارسی میهن جو امکان جستجو و دانلود کتاب های جدید یا قدیمی ایرانی و خارجی را برای شما فراهم می کند" />
<meta name="keywords" content="موتور جستجو, جستجو, میهن جو, میهن, جو, ایران, ایرانی, فارسی, جستجوی فارسی, جستجوی کتاب, دانلود کتاب, دانلود, کتاب, کتاب جدید, کتاب ایرانی" />
...
if(trim($search) != "")
{
 ...
 foreach($d as $index => $value)
 {
	  ...
	  $query = "select * from links
	 	        where url = '$url' and type = 'application/pdf'";
	  $result = mysqli_query($db, $query);
	  $c = mysqli_num_rows($result);
	  for($i=1; $i<=$c; $i++)
	  {
