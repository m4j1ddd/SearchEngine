...
<title><?php if(trim($search) != "") echo $search." - جستجوی "; ?>فیلم های <?php echo $main_title; ?> Videos</title>
<meta name="description" content="بخش فیلم های موتور جستجوی فارسی میهن جو امکان جستجو و دانلود فیلم های جدید یا قدیمی ایرانی و خارجی را برای شما فراهم می کند" />
<meta name="keywords" content="موتور جستجو, جستجو, میهن جو, میهن, جو, ایران, ایرانی, فارسی, جستجوی فارسی, جستجوی فیلم, دانلود فیلم, دانلود, فیلم, فیلم جدید, فیلم ایرانی" />
...
if(trim($search) != "")
{
 ...
 foreach($d as $index => $value)
 {
	  ...
	  $query = "select * from links
	 	       where url = '$url' and type like 'video/%'";
	  $result = mysqli_query($db, $query);
	  $c = mysqli_num_rows($result);
	  for($i=1; $i<=$c; $i++)
	  {
